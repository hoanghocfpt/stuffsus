...
const SignUpPage = () => {
    ...
    const [messageConfirmCheck, setMessageConfirmCheck] = useState('');
    const [submit, setSubmit] = useState([false, false, false, false]);

    const handleValidateFullName = (e) => {
        e.preventDefault();
        const fullName = e.target.value;
        setSubmit(prevSubmit => {
            const newSubmit = [...prevSubmit];
            newSubmit[0] = fullName.length > 0;
            return newSubmit;
        });
        setMessageFullName(fullName.length === 0 ? 'Vui lòng nhập họ và tên' : '');
    }
    ...
    const handleValidateEmail = (e) => {
        ...
        setSubmit(prevSubmit => {
            const newSubmit = [...prevSubmit];
            newSubmit[1] = email.match(emailPattern) !== null;
            return newSubmit;
        });
        setMessageEmail(email.match(emailPattern) ? '' : 'Email không đúng định dạng');
    }
    ...
    const handleValidatePassword = (e) => {
        ...
        setSubmit(prevSubmit => {
            const newSubmit = [...prevSubmit];
            newSubmit[2] = password.length >= 6;
            return newSubmit;
        });
        setMessagePassword(password.length < 6 ? 'Mật khẩu phải lớn hơn 6 ký tự' : '');
    }
    ...
    useEffect(() => {
        ...
        setMessageConfirmCheck(checked ? '' : 'Vui lòng đồng ý với điều khoản');
    }, [checked]);

    const handleSumitSignUp = (e) => {
        e.preventDefault();
        if (submit.every(s => s)) {
        ...
        <div className='relative py-8'>
            ...
            <div className='z-10 max-w-[750px] grid grid-cols-2 py-3 px-3 mx-auto bg-white rounded-2xl'>
                <div className='py-8 px-4 text-center'>
                    ...
                    <h2 className='text-3xl font-semibold'>Bạn đã sẵn sàng?</h2>
                    <p className='font-medium py-2 mb-4'>Đăng ký tài khoản ngay để nhận ưu đãi dành cho người mới</p>
                    <div>
                        <form>
                            <div className={`${messageFullName ? `border-red-200`:`border-gray-200`} flex items-center gap-2 border border-gray-200 rounded-full py-[10px] px-[10px]`}>
                                ...
                                <input onKeyUp={(e)=>handleValidateFullName(e)} className='outline-none border-none text-sm w-full' type='text' placeholder='Họ và tên' />
                            </div>
                            <div className='text-left mb-4'>
                                {messageFullName && (<span className='pl-3 text-red-500 text-sm'>{messageFullName}</span>)}
                            </div>
                            ...
                            <div>
                                <div className='flex justify-between items-center mt-2'>
                                    <div className='flex items-center gap-2'>
                                        <div className='w-5 h-5 flex-shrink-0 overflow-hidden rounded-full flex relative justify-center items-center border border-gray-500'>
                                            <div className={`w-3 h-3 rounded-full absolute ${checked ? `bg-[#444]`:`bg-white`}`}></div>
                                            <input type='checkbox' checked={checked} onChange={()=>setChecked(!checked)} className='w-full opacity-0 h-full object-cover' />
                                        </div>
                                        <span className='text-sm text-left'>Tôi đồng ý với các điều khoản và chính sách của Stuffsus</span>
                                    </div>
                                </div>
                                <div className='text-left mb-4'>
                                    {messageConfirmCheck && (<span className='pl-3 text-red-500 text-sm'>{messageConfirmCheck}</span>)}
                                </div>
                            </div>
                            <button onClick={handleSumitSignUp} className='bg-black text-white w-full py-2 mt-4 rounded-full'>Đăng ký</button>
                        </form>
